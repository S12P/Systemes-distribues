...
ring_node(Father, ChildPid, Manager, Taille_ring, Root) ->
  ...
  %test -> io:fwrite("yoyo~n"), ChildPid ! {broad};
    ...
    {add2, NewChild} -> io:fwrite("add2~p~p~p~n",[self(),NewChild,Father]),
    io:fwrite("add22~p~p~p~n",[self(),NewChild,Father]),
    NewChild ! {broadadd},
    ring_node(self(), NewChild, Manager, Taille_ring, Root);

% faire pour gerer taille ring 
    %{broadadd} -> io:fwrite("Début broadcast~p~n",[ChildPid]), ChildPid ! {broadadd, 0, ChildPid}, ring_node(Father, ChildPid, Manager, Taille_ring+1,Root);
    ...
    {broad, Nb, PID} -> if Taille_ring - Nb == 0 -> io:fwrite("Fin broadcast ~p~n",[ChildPid]), ChildPid ! {add}, ring_node(Father, ChildPid, Manager, Taille_ring,Root);
